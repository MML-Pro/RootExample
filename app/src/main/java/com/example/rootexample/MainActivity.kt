...
// الكود المعدل لزر تغيير عنوان MAC
        binding.changeMacButton.setOnClickListener {
            // استخدام Coroutine لتنفيذ أمر تغيير عنوان MAC
            CoroutineScope(Dispatchers.Main).launch {
                val newMacAddress = generateRandomMacAddress() // توليد عنوان MAC عشوائي
                Log.d(TAG, "Generated MAC Address: $newMacAddress")
    ...
    // دالة لتوليد عنوان MAC عشوائي
    fun generateRandomMacAddress(): String {
        val random = Random()
        val macBytes = ByteArray(6) // عنوان MAC يتكون من 6 بايتات
        random.nextBytes(macBytes) // ملء المصفوفة بقيم عشوائية

        // التأكد من أن البايت الأول ليس متعدد الإرسال (multicast) ومحلي (locally administered)
        macBytes[0] = (macBytes[0].toInt() and 0xFC or 0x02).toByte() // تعيين البت الثاني إلى 1 (locally administered) وإزالة multicast

        // تحويل البايتات إلى صيغة عنوان MAC (مثل 00:11:22:33:44:55)
        return macBytes.joinToString(":") { byte ->
            "%02X".format(byte) // تحويل كل بايت إلى قيمة سداسية عشرية مكونة من رقمين
        }
    }
